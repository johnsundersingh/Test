...
{
    var html;
    html  = "<div class='upProgressContainer'>";
    ...
    html += (progress == 0 ? "Waiting for uploads to start" : "Now uploading <strong>" + file + "</strong> (" + progress + "%)");
    html += "</div>";
    html += "</div>";
    html += "</div>";
   ...
   up_waiting = true;
    ...
    if (up_xhttp.status == 200)
    {
        ...
        else
        {
            ...
            //var bytes = res.documentElement.getAttribute('bytes');
            //var size = res.documentElement.getAttribute('size');
